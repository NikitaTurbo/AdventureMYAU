...
const characters = ['Губка Боб', 'Патрик Стар', 'Гэри', 'Планктон', 'Сквидвард', 'Юджтнн Крабс', 'Сэнди', 'Перл', 'Ларри', 'Баббл Бас'];
const score = localStorage.getItem("score");
let out = Number(score.toString());
let len = 0;
...
if (e.keyCode == 13) {
    ...
    e.preventDefault();
    if (characters.indexOf(val.toString()) >= 0) {
      sendNotification('success', '+1');
      out += 1;
      len += 1;
    }
    else {
      sendNotification('error', 'Character not found!');
      len += 1;
    }
    if (len == 10) {
